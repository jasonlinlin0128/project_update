<template>
  <div class="bg-white p-6 rounded-lg shadow-md mb-8">
    <h3 class="text-xl font-bold mb-4 text-gray-800">活動接洽管理</h3>
    
    <!-- Add Form -->
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6 bg-gray-50 p-4 rounded-lg">
      <input v-model="newItem.name" placeholder="活動/單位名稱" class="border p-2 rounded focus:ring-2 focus:ring-blue-500 outline-none">
      <input v-model="newItem.contact" placeholder="聯絡人" class="border p-2 rounded focus:ring-2 focus:ring-blue-500 outline-none">
      <input v-model="newItem.status" placeholder="接洽狀態" class="border p-2 rounded focus:ring-2 focus:ring-blue-500 outline-none">
      ...
      <table class="w-full text-left border-collapse">
        <thead>
          <tr class="bg-gray-100 text-gray-600 uppercase text-sm leading-normal">
            <th class="py-3 px-6">活動/單位名稱</th>
            <th class="py-3 px-6">聯絡人</th>
        ...
        <tbody class="text-gray-600 text-sm font-light">
          <tr v-for="item in items" :key="item.id" class="border-b border-gray-200 hover:bg-gray-50">
            <template v-if="editingId === item.id">
              ...
              <td class="py-3 px-6"><input v-model="editForm.contact" class="border p-1 rounded w-full"></td>
            ...
            <template v-else>
              ...
              <td class="py-3 px-6">{{ item.contact }}</td>
              <td class="py-3 px-6">
                <span :class="{'bg-green-200 text-green-600': item.status === '已完成', 'bg-yellow-200 text-yellow-600': item.status === '接洽中', 'bg-red-200 text-red-600': item.status === '失敗'}" class="py-1 px-3 rounded-full text-xs">
                  {{ item.status }}
                </span>
              </td>
              <td class="py-3 px-6 text-center">
